...
function App() {
    ...
    try {
      if (callbackCountry) {
        const filteredCountry = countries.filter(
          country =>
            country.name.toLowerCase() === callbackCountry.toLowerCase()
        );

        if (filteredCountry.length) {
          localStorage.setItem("country", JSON.stringify(filteredCountry));
          setCountry(filteredCountry);
        } else throw new Error("No such country in browser");
      } else {
        throw new Error(
          "To check air quality You need to type in country name"
        );
      }
    } catch (error) {
      handleError(error);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await axios.get(
        ...
        country[0].code
        }&parameter=${parameter}&order_by=value&sort=desc&limit=${resultsLimit}&format=json`
      );
      setIsLoading(false);
  ...
  return (
    <div className="container">
      <header>
        <h1>Check air quality in Your country</h1>
      </header>
      <section>
        <AutocompleteInput
