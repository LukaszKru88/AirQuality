    ...
    useEffect(() => {
        const fetchDesctiption = async () => {
            let response = await axios.get(
                `https://en.wikipedia.org/w/api.php?format=json&origin=*&action=opensearch&limit=1&namespace=0&format=json&search=${city}`
            );
            setCityDescription(response.data[3]);
        };

        if (city.length) fetchDesctiption();
    }, [city]);
    ...
    return (
        <div className="row">
            {cities.length ? (
                        ...
                        {cities.map(city => (
                            <Card key={city.location}>
                                ...
                                >
                                    {
                                        <div className="accordion">
                                            ...
                                            <div className="cityPolution">{`PM10: ${city.value} ${
                                                city.unit
                                                }`}</div>
                                        </div>
                                    }
                                ...
                                <Accordion.Collapse eventKey={city}>
                                    <Card.Body>
                                        {cityDescription[0] === "" ? (
                                            `No data found about ${city.city} on Wikipedia`
                                        ) : (
                                                <React.Fragment>
                                                    <span>
                                                        More information about {city.city} under the link:{" "}
                                                    </span>
                                                    <a
                                                        href={cityDescription}
                ...
                <h5 className="offset-3 col-6">
                    <p>No data to view here</p>
                </h5>
            ) : null}
        </div>
    );
